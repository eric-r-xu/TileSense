...
namespace Engine
{
    public abstract class Container
    {
        ...
        private bool _process_paused = false;
        private bool pause_time_set = false;
        private float pause_start_time;
        private float paused_time;
        ...
        protected void do_process(DeltaArgs delta)
        {
            if (process_paused)
            {
                if (!pause_time_set)
                {
                    pause_start_time = delta.time;
                    pause_time_set = true;
                }
                return;
            }

            float process_delta = delta.delta;
            if (pause_time_set)
            {
                paused_time += delta.time - pause_start_time;
                pause_time_set = false;
                process_delta = 0;
            }

            DeltaArgs process_args = delta;
            if (paused_time > 0 || process_delta != delta.delta)
                process_args = new DeltaArgs(delta.time - paused_time, process_delta);

            for (int i = children.size - 1; i >= 0 && i < children.size; i--)
                children[i].do_process(process_args);
            foreach (var animation in animations)
                animation.process(process_args);

            pre_process(process_args);
        ...
        public bool process_paused
        {
            get { return _process_paused; }
            set { _process_paused = value; }
        }
