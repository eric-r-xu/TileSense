...
void main() {
  ...
  test('mixed terminals already includes the all-triplets award', () {
    // Round East pung, discard completes a pung: 4 mixed + 1 wind + 1 closed.
    final open = hkScore('111m 999p EE 111s NN', 'E');
    expect(open.faan, 6);
    expect(open.yaku.any((p) => p.name == 'All Triplets'), isFalse);
    // Discard completes the pair: concealed triplets adds only its upgrade.
    final concealed = hkScore('111m 999p EEE 111s N', 'N');
    expect(
        concealed.yaku
            .singleWhere((p) => p.name == 'All Concealed Triplets (upgrade)')
            .faan,
        5);
    expect(concealed.faan, 11);
  });
  test('all quadruplets replaces the triplet awards', () {
    final score = hkScore('E', 'E', self: true, melds: [
      for (final t in [
        TileType.man2,
        TileType.pin3,
        TileType.sou4,
        ...
        Meld(kind: MeldKind.kan, low: t, concealed: true)
    ]);
    expect(score.yaku.singleWhere((p) => p.name == 'All Quadruplets').faan, 13);
    expect(
        score.yaku.any((p) =>
            p.name == 'All Triplets' || p.name == 'All Concealed Triplets'),
        isFalse);
    expect(score.points, HongKongRules.basePoints(13) * 3);
  });
